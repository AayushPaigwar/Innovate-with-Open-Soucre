...
export default function SecretStats() {
    ...
    const [closedPRCount, setClosedPRCount] = useState(0);
    ...
    useEffect(() => {
        ...
        const fetchClosedPRs = async () => {
            ...
            while (true) {
                try {
                    const response = await fetch(
                        `https://api.github.com/repos/AayushPaigwar/Innovate-with-Open-Soucre/pulls?state=all&page=${page}&per_page=${perPage}`
                    );
                    if (!response.ok) throw new Error("Failed to fetch PR data");
                    const data = await response.json();
                    if (data.length === 0) break; // Exit the loop if no more PRs are found
                    closedPRs = closedPRs.concat(data);
                    page++;
                } catch (error) {
            ...
            setClosedPRCount(closedPRs.length);
        };
        ...
        <section className="py-16 px-4 sm:px-6 lg:px-8 bg-white">
            <div className="max-w-4xl mx-auto text-center">
                <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-gray-900 mb-4">
                    Already {contributorCount}+ contributors & <br /> {closedPRCount}+ PRs Merged Successfully!
                </h2>
            </div>
        </section>
    );
